...
void test_string_all()
{
	...
	test_string_func(); 
	test_string_ifstream_to_string();

	CSring_to_string();

	std::string strSic = "#sic,,get,device.info";
	std::string strRet = sprintf_bin_data((const unsigned char *)strSic.c_str(), strSic.length());
...
int test_string_base()
{
	...
	case 4: {
		...
		while (std::getline(std::cin, s7)) {
			//int len 识别不了？ 改为int
			int len = s7.size(); // size函数返回的是一个std::string::size_type类型的值，
	...
	case 5: {
		// 比较string对象：大小写敏感：==、!=、<、<=、>、>=
		//std::string s1{ "hello" }, s2{ "Hello" }, s3{"Hello world"};//识别不了？
	...
	case 7: {
		...
		*/
		...
		int pos1 = s9.find("Belle");
		int pos2 = s9.find("xxx");
		...
		int pos4 = s10.find_first_not_of(numbers);
		fprintf(stdout, "pos4: %d\n", pos4); // 5
 
		// compare: 返回0(等于)、正数(大于)或负数(小于)
		int ret = numbers.compare(name);
...
int test_string_cctype()
{
 ...
 //
	//for (int &c : s1) { // 对于s1中的每个字符(Note:c是引用)
...
//		for (int it = str.cbegin(); it != str.cend(); ++it)
//			std::cout << *it;
//		std::cout << '\n';
//	}
// 
...
//	{ // crbegin/crend(c++11)
//		std::string str("lorem ipsum");
//		for (int rit = str.crbegin(); rit != str.crend(); ++rit)
//			std::cout << *rit;
//		std::cout << '\n';
//	}
// 
...
//		int i_int = std::stoi(str_auto, nullptr, 0);
// 
//		std::cout << str_dec << ": " << i_dec << " and [" << str_dec.substr(sz) << "]\n";
//		std::cout << str_hex << ": " << i_hex << '\n';
//		std::cout << str_bin << ": " << i_bin << '\n';
//		std::cout << str_int << ": " << i_int << '\n';
//	}
// 
//	{ // stol(c++11)
...
int CSring_to_string()
{
	//char * compare
	{
		char * pch = "invalid_command_error";
		if(pch=="invalid_command_error")
		{
			std::cout << "char * == ok\r";
		}
		else
		...
		const char *pc = pch;
		if(pc=="invalid_command_error")
		{
			std::cout << "const char * == ok\r";
		}
		else
		{
			std::cout << "const char * == no\r";
		}

		if(0==strcmp(pc, "invalid_command_error"))
			std::cout << "strcmp ok\r";
